...
export default function Header({ basics, headerDisplayType }) {
    const iconSize = 18
    ...
    const headerStyle = useMemo(
        () => getCVHeaderDisplayType(headerDisplayType),
        [headerDisplayType]
    );

    const personalStyle = useMemo(
        () => getPersonalDisplayType(headerDisplayType),
        [headerDisplayType]
    );

    const contactStyle = useMemo(
        () => getContactDisplayType(headerDisplayType),
        [headerDisplayType]
    );

    const nameRoleStyle = useMemo(
        () => getNameRoleDisplayType(headerDisplayType),
        [headerDisplayType]
    );
    ...
    return (
        <header className="cv-header" style={headerStyle}>
            <div className="personal" style={personalStyle}>
                {basics?.avatar && (
                    <img src={basics.avatar} alt="avatar" style={avatar} />
                )}
