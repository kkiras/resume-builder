...
export default function SectionDetail({ selectedSection }) {
    ...
    let items = []
    if (selectedSection.id === 'basics') {
        items = [ resumeData.basics ];
    } else {
        items = resumeData.sections.find(
            (section) => section.id === selectedSection.id
        )?.items || [];
    }

    function defaultItemBySection(section) {
        // Tạo mẫu rỗng theo từng section
        switch (section) {
            case "education":
                return {
                id: String(Date.now()),
    ...
    function handleAddNewItem() {
        const newItem = defaultItemBySection(selectedSection.id);
    ...
    // console.log('SectionDetail items:',items)
